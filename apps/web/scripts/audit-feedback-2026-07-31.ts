...
async function main() {
  ...
  const D: Record<string, Entry> = {
    ...
    "FB-1407": { disposition: "blocked", note: "Same root cause as FB-1426: RESEND_API_KEY is not set on Railway (and EMAIL_FROM is the resend.dev sandbox sender).", evidence: () => emailLib.includes("email_not_configured") && emailLib.includes("RESEND_API_KEY") },
    ...
    "FB-1426": { disposition: "blocked", note: "Sending code is correct and refuses to claim an unsent email; RESEND_API_KEY is missing on Railway and EMAIL_FROM is the sandbox sender that can only reach the account owner.", evidence: () => chatRoute.includes("EMAIL NOT SENT") && emailLib.includes("provider_error_") },
