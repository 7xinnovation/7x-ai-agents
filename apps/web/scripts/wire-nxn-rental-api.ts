...
const flow = (customerType: "personal" | "corporate") => {
  ...
  return [
    ...
    `3) Branches: call the Rental/BoxLocations tool with BundleId=<chosen bundle_Id> and EmirateCode=<code>. Then your branch-step reply MUST contain BOTH of these, every single time (this is mandatory, never omit the map): (a) the returned offices as CARDS (nameEn, workingTime), remembering each office's officeId; and (b) IMMEDIATELY AFTER the cards, a fenced block that is exactly three backticks, then a line "map", then a line "emirate: <the EmirateCode you used, e.g. DXB>", then a line "bundle: <the chosen bundle_Id, e.g. MYHOME3>", then a closing line of three backticks. That block renders the "Browse nearby branches on a map" option (location-based nearest branches). Emitting the branch cards WITHOUT the map block is an error — always include both. This is a required UI element, not optional detail, so brevity/conciseness rules do NOT let you skip it.`,
