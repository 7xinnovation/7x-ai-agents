...
const PAYMENT_CONSENT =
  "At payment: if a saved card is on file, use it directly; otherwise capture the card via the payment gateway. Before charging, present the two payment preferences as TOGGLE SWITCHES the customer flips, plus a single proceed button, instead of asking two yes/no questions. Emit a fenced block: three backticks then the word `toggles`, then `title: Before payment`, then `- save_card_consent: Save my card for future payments`, then `- auto_renew_consent: Enable auto-renewal so my box renews automatically next time`, then `confirm: Proceed to payment`, then a closing line of three backticks. When the customer proceeds, record save_card_consent and auto_renew_consent with collect_field from the toggle results, mark auto-renew ACTIVE in the submission if it was on, then call request_payment. Never enable auto-renew without explicit consent. For any other simple yes/no confirmation (for example 'proceed?'), offer a ```buttons block (one `- Label` per option, primary action first) rather than asking the customer to type yes or no.";
