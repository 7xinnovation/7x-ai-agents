"use strict";var DialogEmbed=(()=>{var c="dialog-embed-style";function d(){let e=document.currentScript??document.querySelector("script[data-agent]"),o=e?.dataset??{},t=o.host||new URL(e?.src||location.href).origin;return{agent:o.agent||"default",host:t,locale:o.locale||"en",position:o.position==="bottom-left"?"bottom-left":"bottom-right",uaePassToken:o.uaepassToken||void 0}}function g(e){if(document.getElementById(c))return;let o=e.position==="bottom-left"?"left: 24px;":"right: 24px;",t=`
  :root{--dlg-accent:#2626a1;--dlg-accent-2:#1c1c7d;--dlg-accent-fg:#fff}
  .dlg-launcher{position:fixed;bottom:24px;${o}z-index:2147483000;width:56px;height:56px;border-radius:9999px;border:none;cursor:pointer;padding:0;
  ...
  .dlg-frame.widget{bottom:96px;${o}width:404px;height:640px;max-height:calc(100vh - 120px)}
  ...
  `,i=document.createElement("style");i.id=c,i.textContent=t,document.head.appendChild(i)}async function p(e){try{let o=await fetch(`${e.host}/api/agents/${encodeURIComponent(e.agent)}`);if(!o.ok)return;let t=await o.json(),i=t?.theme?.colors?.primary,a=t?.theme?.colors?.primaryForeground;i&&(document.documentElement.style.setProperty("--dlg-accent",i),document.documentElement.style.setProperty("--dlg-accent-2",`color-mix(in srgb, ${i} 86%, black)`)),a&&document.documentElement.style.setProperty("--dlg-accent-fg",a)}catch{}}var m='<svg class="ic-chat" width="25" height="25" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4c-4.7 0-8.5 3-8.5 6.8 0 1.5.6 2.9 1.6 4.05-.16 1.02-.5 1.98-1.05 2.86-.2.32.02.74.4.72 1.4-.06 2.75-.42 3.9-1.06 1.1.42 2.34.65 3.65.65 4.7 0 8.5-3.05 8.5-6.87S16.7 4 12 4Zm-3.6 7.85a1.2 1.2 0 1 1 0-2.4 1.2 1.2 0 0 1 0 2.4Zm3.6 0a1.2 1.2 0 1 1 0-2.4 1.2 1.2 0 0 1 0 2.4Zm3.6 0a1.2 1.2 0 1 1 0-2.4 1.2 1.2 0 0 1 0 2.4Z"/></svg>',u='<svg class="ic-x" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';function l(){let e=d();g(e),p(e);let o="closed",t=document.createElement("iframe");t.className="dlg-frame widget",t.title="Dialog assistant",t.allow="clipboard-write; microphone";let i=e.uaePassToken?`&upt=${encodeURIComponent(e.uaePassToken)}`:"";t.src=`${e.host}/embed/${encodeURIComponent(e.agent)}?locale=${e.locale}&embedded=1${i}`,window.Dialog={setUaePassToken:n=>t.contentWindow?.postMessage({source:"dialog-host",uaePassToken:n},e.host)};let a=document.createElement("button");a.className="dlg-launcher",a.setAttribute("aria-label","Open assistant"),a.innerHTML=m+u;function r(n){o=n,t.classList.toggle("open",n!=="closed"),t.classList.toggle("full",n==="full"),t.classList.toggle("widget",n==="widget"),a.classList.toggle("open",n!=="closed"),a.style.display=n==="full"?"none":"flex",document.documentElement.style.overflow=n==="full"?"hidden":""}a.addEventListener("click",()=>r(o==="closed"?"widget":"closed")),window.addEventListener("message",n=>{if(n.origin!==e.host)return;let s=n.data;s?.source==="dialog"&&(s.action==="expand"?r("full"):s.action==="collapse"?r("widget"):s.action==="close"&&r("closed"))}),document.body.appendChild(t),document.body.appendChild(a)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",l):l();})();
