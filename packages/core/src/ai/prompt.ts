...
function renderApiFlow(f: NonNullable<NonNullable<import("@dialog/config").Journey["submission"]>["apiFlow"]>): string {
  ...
  lines.push(
    `  3. After the customer confirms, call ${f.saveTool} to create the order. It returns a gateway payment URL (e.g. paymentGateWayResponse.paymentUrl) and a reference number${f.paymentReturnUrl ? ` (use paymentReturnUrl "${f.paymentReturnUrl}")` : ""}. Present that URL as a PAY BLOCK, never as a markdown link: a line of three backticks then the word pay, then \`url: <the exact paymentUrl>\`, then \`amount: <e.g. AED 370.00>\`, then a closing line of three backticks. It renders a button that opens the payment in a popup over the chat; a plain link throws the customer into a new tab and loses the conversation.`
  );
  if (f.confirmTool)
