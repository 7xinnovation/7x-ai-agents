...
| 7 | **UAE PASS redirect URI points at the wrong host** | `UAEPASS_REDIRECT_URI` on Railway is still `https://app-production-207e.up.railway.app/api/uaepass/callback`, but the service now serves from `7xagents.7x-lab.com` (`PUBLIC_APP_URL` / `RAILWAY_PUBLIC_DOMAIN`). The code derives the callback from the origin the customer is actually on, so the env var is only a fallback — but **the origin in use must be the one registered with UAE PASS**, or sign-in fails with `callback.not.match`. | Confirm which host is registered with UAE PASS and make it the single public origin. Then either update `UAEPASS_REDIRECT_URI` to match or drop it (it is unused when `PUBLIC_APP_URL` is set). |
