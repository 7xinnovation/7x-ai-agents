...
| 2 | ~~No hosted deployment~~ **Deployed to Railway 2026-07-08** | Live at `https://web-production-67f24.up.railway.app` (project `7x-ai-agents`, service `web`, Nixpacks via `railway.json`; DB stays on Neon — the app uses the Neon serverless driver). All agents + a live Salesforce chat turn verified on the deployment. **Remaining:** register the Railway callback URL (`…up.railway.app/api/uaepass/callback`) with UAE PASS — until then sign-in on the hosted app fails with `callback.not.match` (local testing on :4500 unaffected). Custom domain TBD. |
