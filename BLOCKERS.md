...
| FB-1426 / FB-1407 | Confirmation emails never arrive | The code was correct and already refused to claim a send it did not make; no provider was configured. **Resolved 2026-08-03** — Resend connected with a verified sender domain and a live send accepted. Remaining: point the internal ops addresses at the real distribution lists (see §2). | Done. |
...
| 4b | **Emirates Post staging serves no PO Box availability** — *Emre confirms 3 Sep 2026 there is now enough staging inventory for testing; the reserved-test-box fallback should be removable, pending one live `Rental/FreeBoxes` check.* | `Rental/FreeBoxes` needs a live EP session and has no inventory behind it in staging, so rental journeys could not get past the box-number step. **Worked around 2026-08-03:** when the availability call returns no numbers, the agent is given a deterministic set of reserved staging test box numbers (gated on the agent's `activeEnvironment`, so it vanishes on a production switch, and only when the real call produced nothing). | **Emirates Post** to either provide a service credential for the availability endpoint or confirm a set of genuinely rentable staging boxes. The workaround is for testing only — it must not be relied on for a production demo of live availability. |
...
| Secret | Status |
|--------|--------|
| `RESEND_API_KEY` | ✅ **Set 2026-08-03 — email is live (FB-1426 / FB-1407 unblocked).** Resend with a verified sender domain (`Agents <no-reply@7x-lab.com>`, set as both `EMAIL_FROM` and `NOTIFICATION_FROM_EMAIL`; the code accepts either). A live send was accepted by the provider. The honesty safeguard is unchanged: the assistant only claims a send the provider confirmed, and offers a resend. |
| `NXN_BRANCH_OPS_EMAIL` / `NXN_EMX_TEAM_EMAIL` | ⚠️ Set, but both point at `moatoum@gmail.com` (a personal test inbox). Now that email actually sends, these must be changed to the real branch-ops and EMX distribution lists before go-live. |
