...
| 4b | **Emirates Post staging serves no PO Box availability** | `Rental/FreeBoxes` needs a live EP session and has no inventory behind it in staging, so rental journeys could not get past the box-number step. **Worked around 2026-08-03:** when the availability call returns no numbers, the agent is given a deterministic set of reserved staging test box numbers (gated on the agent's `activeEnvironment`, so it vanishes on a production switch, and only when the real call produced nothing). | **Emirates Post** to either provide a service credential for the availability endpoint or confirm a set of genuinely rentable staging boxes. The workaround is for testing only — it must not be relied on for a production demo of live availability. |
